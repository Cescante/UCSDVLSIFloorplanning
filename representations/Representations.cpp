...
FPRepresentation::FPRepresentation( int count, int dimensions, int volumn[] )
{
    boxCount = count;
    fpDimensions = dimensions;
    fpVolumn = new int[dimensions];
        ...
        int dimensions[] )
{
    ...
    newBlock.position = new int[fpDimensions];
    newBlock.blockDimensions = new int[fpDimensions];
...
PartialOrdering::PartialOrdering( int count, int dimensions, 
        int volumn[] )
        : InternalRepresentation( count, dimensions, volumn )
{
}
...
{    
    BlockRepresentation blocks( boxCount, fpDimensions, fpVolumn );
    ...
    int* blocksVol = new int[fpDimensions];
    blocksVol[0] = _xOrder.size();
    blocksVol[1] = _yOrder.size();
    blocksVol[2] = _zOrder.size();
